...
import { GP_CONFIG, HOSTS } from '../data/gpData';
...
export const NextProgram: React.FC<NextProgramProps> = ({ onWatchLive, onViewSchedule }) => {
  const { nextLive } = GP_CONFIG;

  return (
    <section id="proximo-programa" className="relative z-20 -mt-8 sm:-mt-12 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
      {/* Section Header Tag */}
      <div className="flex items-center gap-2 mb-3 text-[#AEB6C2] text-xs font-semibold tracking-wider uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          ...
          <div className="lg:col-span-4 flex flex-col items-stretch">
            ...
            <div className="relative w-full rounded-xl overflow-hidden aspect-video group cursor-pointer border border-white/[0.08]" onClick={onWatchLive}>
              ...
              {/* EN VIVO Tag inside thumbnail */}
              <div className="absolute top-2.5 right-2.5 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#E53935] text-white text-[10px] font-bold tracking-wider shadow-lg">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/40">
                <div className="w-10 h-10 rounded-full bg-[#C9A45C] flex items-center justify-center shadow-lg transform scale-90 group-hover:scale-100 transition-transform">
                  <Play className="w-4 h-4 fill-[#070A0F] text-[#070A0F] ml-0.5" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-center">
            ...
            <p className="flex items-center gap-2 text-xs font-semibold text-[#C9A45C] mb-2.5">
              <Calendar className="w-3.5 h-3.5" />
              <span>{nextLive.dateFormatted}</span>
            </p>
